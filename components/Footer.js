...
export default function Example() {
  return (
    <footer className="bg-white border-t-2 lg:border-t mt-4 lg:mt-4">
      <div className="mx-auto max-w-7xl overflow-hidden px-6 py-20 sm:py-16 lg:px-8">
        ...
        >
          {navigation.main.map((item) => (
            <div key={item.name} className="pb-6">
              <Link
                href={item.href}
                className="text-sm leading-6 text-dark hover:text-gray-900 uppercase tracking-wider"
              >
                {item.name}
              </Link>
            </div>
        ...
        <p className="mt-10 text-center text-xs leading-5 text-dark uppercase tracking-wider">
          &copy; 2023 RW EVENTS, Inc. All rights reserved.
        </p>
      </div>
    </footer>
  );
